...
async fn load_announcements_raw() -> Result<AnnouncementResponse, String> {
    // Keep upstream implementations available, but disable all promotional payloads.
    if !super::remote_config::UPSTREAM_REMOTE_SERVICES_ENABLED {
        return serde_json::from_value(serde_json::json!({
            "apiRelayEnabled": false,
            "topRightAdsEnabled": false
