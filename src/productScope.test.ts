...
test('product scope retains complete Codex, Claude and ZCode destinations', () => {
  for(const page of ['codex','codex-api-service','codex-instances','claude','claude-cli','zcode','2fa','settings']) assert.ok(isFocusedPage(page),page);
  assert.ok(isFocusedPlatform('zcode'));
  assert.deepEqual(new Set(MENU_VISIBLE_PLATFORM_IDS),new Set(['codex','codex_api_service','claude_manager','zcode']));
});
